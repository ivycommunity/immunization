<template>
  <NavBar />
  <div class="mt-28 min-h-screen bg-[#F8F8FF] text-[#121212] py-12 px-6">
    <section class="max-w-6xl mx-auto">
      <h2 class="text-4xl font-bold text-center mb-12">Find Nearby Clinics</h2>

      <div class="flex flex-col md:flex-row justify-between items-center mb-8 space-y-4 md:space-y-0">
        <input v-model="searchQuery" type="search" placeholder="Search clinics by name or location..."
      ...
      <div class="bg-[#121212] rounded-lg overflow-hidden h-[500px] relative">
        ...
        <div v-if="selectedClinic" class="absolute top-4 right-4 bg-[#F8F8FF] p-6 rounded-lg max-w-sm w-full">
          <button @click="selectedClinic = null" class="absolute top-2 right-2 text-[#121212] hover:text-[#04A699]">
            ×
          </button>
      ...
      <div class="mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="clinic in filteredClinics" :key="clinic.id"
